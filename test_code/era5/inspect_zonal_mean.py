...
file = "AAM_data/monthly_mean/zonal_mean_ERA5_u_1994-09.nc"
output_path = "AAM_data/monthly_mean/figs"
...
import os
import xarray as xr
base_path = "AAM_data/monthly_mean/"
...
for year in range(start_year, end_year + 1):
    for month in range(1,13):
        ...
        filename_lnsp = f"zonal_mean_ERA5_lnsp_{year:04d}-{month:02d}.nc"
        filepath_lnsp = os.path.join(base_path, filename_lnsp)
        try:
            ds = xr.open_dataset(filepath_lnsp)
            _ = ds.surface_pressure_zonal_mean.mean().values  # attempt to access data
        except Exception as e:
            print(f"Invalid file: {filename_lnsp} -- Error: {e}")
