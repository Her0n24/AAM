...
def plot_AAM_anomalies(start_year, end_year, component='AAM', *, nlevels=11, cmap_name='RdBu_r', 
    ...
    if max_abs_value >= 1e3 or max_abs_value <= 1e-3:
        ...
        cbar.set_ticks(levels[::max(1, len(levels)//8)].tolist())
        cbar.set_ticklabels([f'{val/factor:.1f}' for val in levels[::max(1, len(levels)//8)]])  # type: ignore[arg-type]
    else:
...
plot_AAM_anomalies(start_yr, end_yr, component='AAM', 
                   savefile=f'AAM_anomalies_{ensemble_member}_{start_yr}-{end_yr}_{args.p_min}-{args.p_max}hPa_wENSO.png', 
                   show=True)#, vmin=-0.3e24, vmax=0.3e24)
