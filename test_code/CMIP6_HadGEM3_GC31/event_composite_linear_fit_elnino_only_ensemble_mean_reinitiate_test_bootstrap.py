...
output_dir = f"{base_dir}/figures/may_2026/"
...
ensemble_results_dir = f"{base_dir}/event_composite_ensemble_mean_results/"  # Store composites, significance, active-month data
...
def save_composite_results(
    ...
    ens_mean : xr.DataArray
        Ensemble mean field
    p_vals : np.ndarray
        P-values from significance testing (same shape as ens_mean)
    ...
    filename = f"composite_{composite_type}_{strength_str}_onset_{args.onset_season}_start_{args.composite_start}_region_{args.region}_ensemble_results.nc"
    ...
    ds['p_values'] = p_vals_da

    # Prepare a boolean significance mask for a separate file
    significance_mask = np.asarray(p_vals <= 0.05, dtype=bool)
    ...
    print(f"Saved composite results to {filepath}")

    mask_filename = f"composite_{composite_type}_{strength_str}_onset_{args.onset_season}_start_{args.composite_start}_region_{args.region}_significance_mask.nc"
    mask_filepath = os.path.join(results_dir, mask_filename)
    mask_ds = xr.Dataset({'significance_mask': significance_mask_da})
    mask_ds.attrs['ensemble_n_members'] = int(ens_stack.sizes['ensemble'])
    mask_ds.attrs['composite_type'] = str(composite_type)
    mask_ds.attrs['strength_label'] = str(strength_label)
    mask_ds.attrs['significance_threshold'] = 0.05
    if metadata is not None:
        for key, val in metadata.items():
            try:
                mask_ds.attrs[f'meta_{key}'] = str(val)
            except:
                pass
    mask_ds.to_netcdf(mask_filepath)
    print(f"Saved boolean significance mask to {mask_filepath}")
...
def load_composite_results(
    ...
    filename = (
        f"composite_{composite_type}_{strength_str}_onset_{onset_season}"
        f"_start_{composite_start}_region_{region}_ensemble_results.nc"
    )
    filepath = os.path.join(results_dir, filename)
    legacy_filepath = os.path.join(results_dir, f"composite_{composite_type}_{strength_str}_ensemble_results.nc")
    ...
    if not os.path.exists(filepath):
        if os.path.exists(legacy_filepath):
            ...
            print(f"Legacy results file not found: {legacy_filepath}")
            return None
    
    try:
        ds = xr.open_dataset(filepath)
        results = {
            'ens_mean': ds['ens_mean'].values if 'ens_mean' in ds else None,
            'p_values': ds['p_values'].values if 'p_values' in ds else None,
            'significance_mask': ds['significance_mask'].values if 'significance_mask' in ds else None,
...
def compute_and_save_composite_significance(
    ...
    """Compute significance (p-values) for a composite and save results.
    ...
    -------
    Tuple[np.ndarray, str]
        (p_values array, filepath to saved results)
    """
    # Compute ensemble mean
    ens_mean = ens_stack.mean("ensemble", skipna=True)
    
    # DIAGNOSTIC: Print ensemble and data info
    print(f"\n[SIGNIFICANCE DEBUG - {composite_type}]")
    print(f"  ens_stack dims: {ens_stack.dims}, shape: {ens_stack.shape}")
    print(f"  ens_stack['ensemble'] size: {ens_stack.sizes.get('ensemble', '?')}")
    print(f"  ens_mean shape: {ens_mean.shape}")
    print(f"  ens_stack data range: [{np.nanmin(ens_stack.values):.2e}, {np.nanmax(ens_stack.values):.2e}]")
    print(f"  ens_mean data range: [{np.nanmin(ens_mean.values):.2e}, {np.nanmax(ens_mean.values):.2e}]")
    
    # t-test only when not replotting
    if replot:
        ...
        p_vals = loaded_results['p_values']
        if p_vals is None:
            raise RuntimeError("P-values not found in loaded results for replotting.")
        return p_vals, "loaded_from_saved_results"
    
    else:
        # Perform t-test
        ttest_result = _stats.ttest_1samp(ens_stack.values, 0.0, axis=0, nan_policy="omit")
        p_vals = np.asarray(ttest_result.pvalue, dtype=float)
        
        # DIAGNOSTIC: Check t-statistic distribution
        t_stat = np.asarray(ttest_result.statistic, dtype=float)
        print(f"  t-statistic range: [{np.nanmin(t_stat):.2f}, {np.nanmax(t_stat):.2f}]")
        print(f"  p-value range: [{np.nanmin(p_vals):.4f}, {np.nanmax(p_vals):.4f}]")
        
        # CRITICAL: Check for NaN distribution
        nan_count_ensemble = np.isnan(ens_stack.values).sum()
        total_points = ens_stack.values.size
        print(f"  NaN values in ens_stack: {nan_count_ensemble} / {total_points} ({100*nan_count_ensemble/total_points:.1f}%)")
        
        # Check mean and std of actual data
        data_mean = np.nanmean(ens_stack.values)
        data_std = np.nanstd(ens_stack.values)
        data_sem = data_std / np.sqrt(ens_stack.sizes['ensemble'])  # Standard error of mean
        print(f"  Data statistics (overall):")
        print(f"    Mean: {data_mean:.2e}, Std: {data_std:.2e}, SEM: {data_sem:.2e}")
        
        # For each point, compute approximate effect size (mean / std per point)
        # This helps diagnose if signal is just too small relative to noise
        ensemble_axis = list(ens_stack.dims).index('ensemble')
        point_means = np.nanmean(ens_stack.values, axis=ensemble_axis)
        point_stds = np.nanstd(ens_stack.values, axis=ensemble_axis)
        effect_sizes = np.abs(point_means) / (point_stds + 1e-10)  # Avoid division by zero
        print(f"  Effect size (|mean|/std) statistics:")
        print(f"    Range: [{np.nanmin(effect_sizes):.2e}, {np.nanmax(effect_sizes):.2e}]")
        print(f"    Mean: {np.nanmean(effect_sizes):.2e}, Median: {np.nanmedian(effect_sizes):.2e}")
        
        # Prepare metadata
        metadata = {
            'enso_state': getattr(args, 'enso_state', 'el_nino') if args else 'el_nino',
            'start_year': getattr(args, 'start_year', None) if args else None,
            'end_year': getattr(args, 'end_year', None) if args else None,
            'p_min': getattr(args, 'p_min', None) if args else None,
            'p_max': getattr(args, 'p_max', None) if args else None,
        }
...
def _plot_and_save_ensemble_mean_aam_composite(
    *,
    ...
    if not replot:
        ...
        p_vals_saved, save_path = compute_and_save_composite_significance(
            ens_stack=ens_stack,
            composite_type='aam',
            strength_label=strength_label,
        ...
        insig_mask = np.where((p_vals > 0.05) | np.isnan(p_vals), 1, 0)
    else:
        # Replot Mode: Load the boolean mask directly
        mask_filename = f"composite_aam_{strength_label}_onset_{args.onset_season}_start_{args.composite_start}_region_{args.region}_significance_mask.nc"
        mask_filepath = os.path.join(ensemble_results_dir, mask_filename)
        
        if os.path.exists(mask_filepath):
            print(f"  Loading boolean significance mask from {mask_filename}")
            sig_mask = xr.open_dataset(mask_filepath)["significance_mask"].values
            # The saved mask is True for significant. We want 1 for INSIGNIFICANT to hatch it.
            insig_mask = np.where(sig_mask, 0, 1)
            insig_mask = insig_mask.T
        else:
            print(f"  WARNING: Mask not found at {mask_filepath}. Hatching everything.")
...
if __name__ == '__main__':
    ...
    if not replot:
        ...
        # --- 1. Load Metadata & Significance from the RESULTS file ---
        # This is the file you ncdump-ed earlier; it contains the count and active %
        main_results_path = os.path.join(
            ensemble_results_dir,
            f"composite_aam_all_onset_{args.onset_season}_start_{args.composite_start}_region_{args.region}_ensemble_results.nc",
        )
        
        if os.path.exists(main_results_path):
            with xr.open_dataset(main_results_path) as ds_res:
                # Get the member count from global attributes
                if 'ensemble_n_members' in ds_res.attrs:
                    number_of_available_members = int(ds_res.attrs['ensemble_n_members'])
                
                # Get the active month profile for the plot labels
                if 'active_month_percent' in ds_res:
                    active_month_percent = ds_res['active_month_percent'].values
            print(f"  Extracted metadata: {number_of_available_members} members.")
        else:
            print(f"  WARNING: Results file not found at {main_results_path}. Metadata will be missing.")

        # --- 2. Load Main AAM Composite ---
        ...
        if os.path.exists(main_nc_path):
            ...
            main_mask_path = os.path.join(
                ensemble_results_dir,
                f"composite_aam_all_onset_{args.onset_season}_start_{args.composite_start}_region_{args.region}_significance_mask.nc",
            )
            if os.path.exists(main_mask_path):
                significance_mask_aam = xr.open_dataset(main_mask_path)["significance_mask"].values
            else:
                significance_mask_aam = None
        else:
            main_results_fallback = os.path.join(
                ensemble_results_dir,
                f"composite_aam_all_onset_{args.onset_season}_start_{args.composite_start}_region_{args.region}_ensemble_results.nc",
            )
            if os.path.exists(main_results_fallback):
                with xr.open_dataset(main_results_fallback) as ds_main:
                    if 'ens_mean' in ds_main:
                        ensemble_composites.append(ds_main['ens_mean'])
                        print(f"  Fallback-loaded main AAM mean from {main_results_fallback}")

        # --- 3. Load LatLev Composite & Mask ---
        ...
        if os.path.exists(latlev_nc_path):
            ...
            latlev_mask_path = os.path.join(
                ensemble_results_dir,
                f"composite_latlev_all_onset_{args.onset_season}_start_{args.composite_start}_region_{args.region}_significance_mask.nc",
            )
            if os.path.exists(latlev_mask_path):
                significance_mask_latlev = xr.open_dataset(latlev_mask_path)["significance_mask"].values
            else:
                significance_mask_latlev = None
                
            u_latlev_path = os.path.join(ensemble_mean_output_path, f"U_lat_lev_composite_ENSEMBLE_MEAN_{args.enso_state}_state_{args.start_year}-{args.end_year}_{args.p_min}-{args.p_max}hPa_{args.onset_season}_start_{args.composite_start}_region_{args.region}{reinitiation_suffix}.nc")
            if os.path.exists(u_latlev_path):
                u_ds = xr.open_dataset(u_latlev_path)
                # CRITICAL: Use .append() so the list isn't empty!
                ensemble_u_latlev.append(u_ds["u"]) 
                print(f"  Loaded wind overlay from {u_latlev_path}")
        else:
            latlev_results_fallback = os.path.join(
                ensemble_results_dir,
                f"composite_latlev_all_onset_{args.onset_season}_start_{args.composite_start}_region_{args.region}_ensemble_results.nc",
            )
            if os.path.exists(latlev_results_fallback):
                with xr.open_dataset(latlev_results_fallback) as ds_latlev:
                    if 'ens_mean' in ds_latlev:
                        ensemble_lat_lev_composites.append(ds_latlev['ens_mean'])
                        print(f"  Fallback-loaded lat×level mean from {latlev_results_fallback}")
                
        # 4. Load LatLon Composite & Mask
        ...
        latlon_mask_path = os.path.join(
            ensemble_results_dir,
            f"composite_latlon_all_onset_{args.onset_season}_start_{args.composite_start}_region_{args.region}_significance_mask.nc",
        )
        if os.path.exists(latlon_nc_path):
            ...
            if os.path.exists(latlon_mask_path):
                significance_mask_latlon = xr.open_dataset(latlon_mask_path)["significance_mask"].values
            else:
                significance_mask_latlon = None
                
            u_latlon_path = os.path.join(ensemble_mean_output_path, f"U_lat_lon_composite_ENSEMBLE_MEAN_{args.enso_state}_state_{args.start_year}-{args.end_year}_{args.p_min}-{args.p_max}hPa_{args.onset_season}_start_{args.composite_start}_region_{args.region}{reinitiation_suffix}.nc")
            if os.path.exists(u_latlon_path):
                ensemble_u_latlon.append(xr.open_dataset(u_latlon_path)["u"])
                
            uv_latlon_path = os.path.join(ensemble_mean_output_path, f"UV_lat_lon_composite_ENSEMBLE_MEAN_{args.enso_state}_state_{args.start_year}-{args.end_year}_{args.p_min}-{args.p_max}hPa_{args.onset_season}_start_{args.composite_start}_region_{args.region}{reinitiation_suffix}.nc")
            if os.path.exists(uv_latlon_path):
                ensemble_uv_vi_lat.append(xr.open_dataset(uv_latlon_path)["uv"])
        else:
            latlon_results_fallback = os.path.join(
                ensemble_results_dir,
                f"composite_latlon_all_onset_{args.onset_season}_start_{args.composite_start}_region_{args.region}_ensemble_results.nc",
            )
            if os.path.exists(latlon_results_fallback):
                with xr.open_dataset(latlon_results_fallback) as ds_latlon:
                    if 'ens_mean' in ds_latlon:
                        ensemble_lat_lon_composites.append(ds_latlon['ens_mean'])
                        print(f"  Fallback-loaded lat×lon mean from {latlon_results_fallback}")
        ...
        for strength_label, _, _ in EVENT_STRENGTH_BINS:
            ...
            if os.path.exists(strength_nc_path):
                ensemble_composites_by_strength[strength_label].append(xr.open_dataset(strength_nc_path)["AAMA"])
            else:
                strength_results_path = os.path.join(
                    ensemble_results_dir,
                    f"composite_aam_{strength_label}_onset_{args.onset_season}_start_{args.composite_start}_region_{args.region}_ensemble_results.nc",
                )
                if os.path.exists(strength_results_path):
                    with xr.open_dataset(strength_results_path) as ds_strength:
                        if 'ens_mean' in ds_strength:
                            ensemble_composites_by_strength[strength_label].append(ds_strength['ens_mean'])
                        if 'active_month_percent' in ds_strength:
                            strength_active_month_percent_by_label[strength_label] = ds_strength['active_month_percent'].values
                            print(f"  Loaded active_month_percent for {strength_label} from {strength_results_path}")
    ...
    if ensemble_composites:
        try:
            ...
            if not replot:
                combined = ens_stack.transpose("ensemble", lat_dim, "month")
                _, p_vals = _stats.ttest_1samp(combined.values, 0.0, axis=0, nan_policy="omit")
                ...
                # Save AAM all-event composite significance
                p_vals_aam, aam_save_path = compute_and_save_composite_significance(
                    ens_stack=ens_stack,
                    composite_type='aam',
                    strength_label='all',
                    results_dir=ensemble_results_dir,
                ...
                # 1 = Insignificant (hatch it)
                insig_mask = np.where((p_vals > 0.05) | np.isnan(p_vals), 1, 0)
            else:
                if 'significance_mask_aam' in locals() and significance_mask_aam is not None:
                    # Invert the boolean mask: True (significant) becomes 0. False becomes 1 (hatch).
    ...
    if ensemble_lat_lev_composites:
        try:
            ...
            ens_mean = ens_stack.mean("ensemble", skipna=True)
            if not replot:
                # Compute significance for lat-lev composite
                p_vals_latlev, _ = compute_and_save_composite_significance(
                    ens_stack=ens_stack,
    ...
    if ensemble_lat_lon_composites:
        try:
            ...
            if not replot:
                # Compute significance for lat-lon composite
                p_vals_latlon, _ = compute_and_save_composite_significance(
                    ens_stack=ens_stack,
